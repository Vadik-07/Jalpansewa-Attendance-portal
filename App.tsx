...
export default function App() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (sewadarsData) setSewadars(sewadarsData);
  ...
  const handleAddMember = async (name: string) => {
    try {
      ...
      if (data) setSewadars(prev => [data, ...prev].sort((a,b) => a.name.localeCompare(b.name)));
  ...
  const handleUpdateMember = async (id: string, newName: string) => {
    try {
      const { error } = await supabase.from('sewadars').update({ name: newName }).eq('id', id);
      if (error) throw error;
      setSewadars(prev => prev.map(s => s.id === id ? { ...s, name: newName } : s));
    } catch (error) { alert('Failed to update member.'); }
  };
  ...
  const handleMarkOut = async (recordId: string, endTime: string) => {
    ...
    } catch (error) { alert('Failed to mark out.'); }
  };
